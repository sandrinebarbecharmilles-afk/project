...
(function () {
  ...
  function viewImport(space) {
    ...
    $('#content').innerHTML = `
      ...
      <div class="info-box">
        Le fichier (<strong>.csv</strong> ou <strong>.xlsx</strong>) doit être un tableau « une ligne = un évènement », avec une <strong>ligne d'en-tête</strong>. Colonnes reconnues (FR ou EN) :
        ...
        Dates : <code>2026-09-15</code> ou <code>15/09/2026</code>. Pour Excel, la <strong>première feuille</strong> est utilisée.
  ...
  function previewFromRows(allRows, space) {
    ...
    for (let i = 1; i < rows0.length; i++) {
      ...
      rows.push({ id: uid(), date, title, time: iTime >= 0 ? String(cols[iTime] || '').trim() : '', memberId: matchMember(memberName), _memberName: memberName, type: iType >= 0 ? String(cols[iType] || '').trim() : '', note: iNote >= 0 ? String(cols[iNote] || '').trim() : '' });
    }
    csvParsed = rows;
